function ode_findreplace(ex, symex, indvar_dict, params; params_from_function = true,
    ...
    for (i, arg) in enumerate(ex.args)
        ...
        elseif isa(arg, Symbol)
            ...
            elseif length(string(arg)) > 1 && haskey(indvar_dict, Symbol(s[nextind(s, 1):end])) &&
                   Symbol(s[1]) == :d
...
function ode_symbol_findreplace(ex, indvar_dict, params; params_from_function = true)
    ...
    return :($ex * 1) # Add the 1 to make it an expression not a Symbol
end
    ...
    for (i, arg) in enumerate(ex.args)
        if isa(arg, Expr)
            if arg.args[1] == :(*) && length(arg.args) >= 3 &&
               (isa(arg.args[2], Number) || (isa(arg.args[2], Expr) && arg.args[2].args[1] == :-))
